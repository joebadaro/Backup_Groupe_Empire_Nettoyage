...
export interface EstimationData {
    ...
    clientPhone: string;
    clientCity: string;
    clientAddress?: string; // New Address Field
    clientNotes?: string;
    items: {
        label: string;
        price: number;
...
export const generateEstimationPDF = async (data: EstimationData) => {
    ...
    if (data.clientCity) {
        doc.text(data.clientCity, textX, textY);
        textY += 5;
    }
    ...
    const disclaimer = isFr
        ? "Le prix indiqué inclut notre service principal de nettoyage à la vapeur et au shampoing, utilisé dans la grande majorité des interventions.\n\nAvant le début des travaux, le technicien effectuera une vérification afin de confirmer que la méthode appliquée est adéquate pour le tissu ou la surface. Dans certaines situations, l’utilisation d’un produit ou d’une méthode différente peut s’avérer plus appropriée, notamment lorsque le matériau requiert des soins plus spécialisés. Le technicien vous expliquera exactement ce dont vous avez besoin et vous déciderez ensemble avant le début du travail."
        : "The indicated price includes our main steam cleaning and shampooing service, used in the vast majority of interventions.\n\nBefore work begins, the technician will perform a verification to confirm that the applied method is suitable for the fabric or surface. In certain situations, the use of a different product or method may prove more appropriate, especially when the material requires more specialized care. The technician will explain exactly what is needed, and you will decide together before the work begins.";
    ...
    try {
        ...
        // Attempt to open in new tab for better mobile experience
        try {
